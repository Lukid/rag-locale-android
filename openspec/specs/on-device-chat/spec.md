# on-device-chat Specification

## Purpose

Offrire una chat conversazionale solo testo che gira interamente on-device, con risposta in streaming token-per-token, cronologia di sessione come contesto, limite configurabile sui token di output e possibilità di interrompere la generazione in corso.
